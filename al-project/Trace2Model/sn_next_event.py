...
def post_process(output,next_event,data_type,trace_dict,enum_val,temp_var):
	...
	for j in e_int:
		...
		for x in expr_split:
			# 임시 변수 제거
			contain_temp_vars = [is_contain_temp(x_expr, temp_var) for x_expr in x]
			if True in contain_temp_vars:
				continue
			if len(x) > 1:
...
def is_contain_temp(output, temp_vars):
	output_copy = str(output)
	characters = ['+','-','*','<=','>=','=','>','<','!','(',')']
	
	for char in characters:
		output_copy = output_copy.replace(char, "")

	output_vars = output_copy.split()
	contain_vars = list(set(output_vars) & set(temp_vars))
	return len(contain_vars) > 0
...
def gen_syn(input_dict,trace_dict):
	...
	for i in trace_set.keys():
		...
		for j in next_event:
			...
			output = post_process(output,[0,1],data_type,trace_dict,enum_val, ["determineSpeed$$j$$1"])
			trace_events[i][j] = output['1']

			if trace_events[i][j] != 0:
				print(colored(j + ': ' + trace_events[i][j],'green'))

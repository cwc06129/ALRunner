...
)
{
  ...
  tpl_service.parameters.id.task_id = (((const tpl_task_activation_action * )action)->task_id);
  result_action = tpl_activate_task( ((const tpl_task_activation_action * )action)->task_id);
  if ((0x1F & (result_action)) != 0) { tpl_call_error_hook(result_action); }
    ...
    const tpl_action * action)
{
  ...
  tpl_service.parameters.id.task_id = (((const tpl_setevent_action * )action)->task_id);
  tpl_service.parameters.param.mask = (((const tpl_setevent_action * )action)->mask);
  result_action = tpl_set_event(
      ((const tpl_setevent_action * )action)->task_id,

...
extern tpl_bool tpl_cpt_os_task_lock;
 const int signal_for_counters = 31;
sigset_t signal_set;
void tpl_call_counter_tick();
void tpl_enable_interrupts(void)
{
    if ( -1 == sigprocmask(2, &signal_set, ((void *)0)) )
    {
...
{
 //((tpl_locking_depth > 0) ? (void)0 : __assert_func (".\\source\\tpl_machine_posix.c", 288, __FUNCTION__, "tpl_locking_depth > 0"));
 assert(tpl_locking_depth > 0) ;
 tpl_locking_depth--;
    tpl_cpt_os_task_lock--;
    if ( (tpl_locking_depth == 0) && (((tpl_bool)0) == tpl_user_task_lock) )
    {
        if (sigprocmask(2,&signal_set,((void *)0)) == -1) {
...
void tpl_osek_func_stub( tpl_proc_id task_id )
{
 ...
 tpl_release_task_lock();
  ...
  TerminateISR();
        ...
        //fprintf(((__getreent())->_stderr), "[OSEK/VDX Spec. 2.2.3 Sec. 4.7] Ending the task without a call to TerminateTask or ChainTask is strictly forbidden and causes undefined behaviour.\n");
        exit(1);
    }
}
sigset_t saved_mask;
    ...
    const tpl_proc_id proc_id)
{
    ...
    kill(getpid(), 30);
    sigfillset(&new_mask);
    sigdelset(&new_mask, 30);
	int i=1;//extra
	while((((tpl_bool)0) == handler_has_been_triggered)&&i!=2){
        sigsuspend(&new_mask);
		i++;//extra
	}//extra
    ...
    sigaction(30, &old_action, ((void *)0));
    sigprocmask(0, &old_mask, ((void *)0));
    if ( 0 == _setjmp(tpl_stat_proc_table[(2 +0)]->context->current) )
...
void tpl_init_machine(void)
{
	...
	/****Extra Added to pass 31 as tpl_signal_handler parameter****/
	void (*foo)(int);
	foo = &tpl_signal_handler;
	foo( 31 );
	/****Extra Added to pass 31 as tpl_signal_handler parameter****/

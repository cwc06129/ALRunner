...
public class Expression {
	...
	/*chl@2023.02.09 - make next expression var to find next line for not branch but nested*/
	private Expression next;
	
	/*chl@2023.02.08 - visit checking for code print*/
	public boolean visit = false;
	
	protected ArrayList<String> castType = new ArrayList<String>(Arrays.asList("char", "short", "int", "long", "float", "enum", "unsigned", "signed"));
	...
	public ArrayList<Expression> makeExprFromLex(ArrayList<UdbLexemeNode> lexemes, HashMap<UdbLexemeNode, Integer> theNumberOfFunctionParameter, HashMap<Function, Boolean> typeOfFunc, ArrayList<String> defines){
		...
		for (int i = 0 ; i<lexemes.size() ; i++){
			if (lexemes.get(i).getToken().equals("Whitespace")) {
				...
				i--;
			} else if (i == lexemes.size()-1 && lexemes.get(i).getToken().equals("Punctuation") && lexemes.get(i).getData().equals(";")){
				lexemes.remove(lexemes.get(i));
				break;
			}
	}
		...
		*/	
		...
		for (int i = 0 ; i<lexemes.size() ; i++){
			...
			/*1-1. Identifier(lexeme - Token) handles Call, Type, Declare, Define (lexeme - Ref_Kind)
			...
			*/
			...
			if (lexeme.getToken().equals("Identifier")) {
				...
				if (lexeme.getRef_kind().equals("Call")) {
					 ...
					 * librarycall: {pseudoExpression - nullExpression - pseudoExpression} -> Calltype: true
				...
				/* For IdExpression Assignment */
					...
					if(lexeme.getVar() != null && lexeme.getVar().getType().equals("enum") == false) {
						...
						if (lexeme.getVar().getType().toString().contains("struct")){
							ArrayList<String> str = new ArrayList<>(Arrays.asList("const", "static", "unsigned", "signed", "extern")); 
							if (i-4>=0 && (str.contains(lexemes.get(i-4).getData()) && str.contains(lexemes.get(i-3).getData()))){
						...
						//enum ì¶”ê??˜ê¸°
						if (lexeme.getVar().getType().contains("const") && lexeme.getVar().getType().contains("struct") ) {
						...
						if (lexeme.getToken().equals("Identifier") && lexeme.getRef_kind().equals("Declare") && i-1>=0 && lexemes.get(i-1).getToken().equals("Operator") && i-2 >=0 && expressions.get(i-2) instanceof CastExpression){
							idExpression = new IdExpression(lexeme.getData(), lexeme.getVar().getScope(), ((CastExpression)expressions.get(i-2)).getType().toString(), lexeme.getVar());
						} else {
							type += lexeme.getVar().getType();
							idExpression = new IdExpression(lexeme.getVar().getName(), lexeme.getVar().getScope(), type, lexeme.getVar());
						}
					}
					else if (lexeme.getVar() == null && i-1>=0 && lexemes.get(i-1).getToken().equals("Identifier") && lexemes.get(i-1).getRef_kind().equals("Type")){
						ArrayList<String> str = new ArrayList<>(Arrays.asList("const", "static", "unsigned", "signed", "extern"));
						String type = "";
						if(i-4>=0 && str.contains(lexemes.get(i-4).getData()) && str.contains(lexemes.get(i-3).getData()) && str.contains(lexemes.get(i-2).getData())){
							type += lexemes.get(i-4).getData() + " ";
							type += lexemes.get(i-3).getData() + " ";
						...
						else if(i-3>=0 && str.contains(lexemes.get(i-3).getData()) && str.contains(lexemes.get(i-2).getData())){
							type += lexemes.get(i-3).getData() + " ";
							type += lexemes.get(i-2).getData() + " ";
							lexemes.remove(i-2);
							lexemes.remove(i-3);
					...
					} else if (lexeme.getRef_kind().equals("Define") && i-1>=0 && (lexemes.get(i-1).getData().contains("struct") || lexemes.get(i-1).getData().contains("enum"))) {
						...
						if (typedef == true){
							...
							tdefExpr.setType(lexemes.get(i-1).getData());
							tdefExpr.setName(lexeme.getData());
							if (lexemes.get(lexemes.size()-1).getVar() != null && lexemes.get(lexemes.size()-1).getVar().getType().equals("enum")){
								tdefExpr.setNick(lexemes.get(lexemes.size()-1).getData());
								lexemes.remove(lexemes.size()-1);
							}
						}
						...
						continue;
					} else if (lexeme.getRef_kind().equals("Define") && typedef==true && (lexemes.get(1).getData().contains("struct") || lexemes.get(1).getData().contains("enum"))){
				...
				*/
				if (lexeme.getData().equals("[")) {
					...
					Operator Classification -> Unary Operation, Binary Operation, Ternary Operation
				...
				} else if (operatorInfo.getBinaryOperator().contains(lexemes.get(i).getData()) || operatorInfo.getTernaryOperator().contains(lexemes.get(i).getData())) {
				...
				if (lexeme.getData().equals("continue") || lexeme.getData().equals("return") || lexeme.getData().equals("goto") || lexeme.getData().equals("break")){
					ControlExpression controlExpression = new ControlExpression();
					controlExpression.setFunctionalName(lexeme.getData());
					expressions.add(controlExpression);
				} /* 
				...
				*/ 
				else if (i-1>=0 && lexemes.get(i-1).getData().equals("(") && i+1<lexemes.size() && lexemes.get(i+1).getData().equals(")")){ // cast to unary
					UnaryExpression unaryExpr = new UnaryExpression();
					unaryExpr.setOperator(lexemes.get(i-1).getData()+lexemes.get(i).getData()+lexemes.get(i+1).getData());
					expressions.remove(expressions.size()-1);
					lexemes.remove(i+1);
					expressions.add(unaryExpr);
				} else if (i-1>=0 && lexemes.get(i-1).getData().equals("(") && i+1<lexemes.size() && lexemes.get(i+1).getData().equals("*") && i+2<lexemes.size() && lexemes.get(i+2).getData().equals(")")) {
					UnaryExpression unaryExpr = new UnaryExpression();
					unaryExpr.setOperator(lexemes.get(i-1).getData()+lexemes.get(i).getData()+lexemes.get(i+1).getData()+lexemes.get(i+2).getData());
					expressions.remove(expressions.size()-1);
					lexemes.remove(i+2);
					lexemes.remove(i+1);
					expressions.add(unaryExpr);
				} else if (i-2>=0 && lexemes.get(i-2).getData().equals("(") && lexemes.get(i-1).getData().equals("*") && i+1<lexemes.size() && lexemes.get(i+1).getData().equals(")")) {
					UnaryExpression unaryExpr = new UnaryExpression();
					unaryExpr.setOperator(lexemes.get(i-2).getData()+lexemes.get(i-1).getData()+lexemes.get(i).getData()+lexemes.get(i+1).getData());
					expressions.remove(expressions.size()-1);
					expressions.remove(expressions.size()-1);
					lexemes.remove(i+1);
					expressions.add(unaryExpr);
				...
				else if (lexeme.getData().equals("const") || lexeme.getData().equals("static") || lexeme.getData().equals("extern") || lexeme.getData().contains("struct") || lexeme.getData().contains("enum") ||  lexeme.getData().equals("unsigned") || lexeme.getData().contains("typedef")){
					continue;
				} /* const, static, extern, struct, unsigned -> will be absorbed to the latter CastExpression */ 
				else {
					ArrayList<String> str = new ArrayList<>(Arrays.asList("const", "static", "unsigned", "signed", "extern"));
					String data = "";
					CastExpression castExpression;
					if(i-3>=0 && str.contains(lexemes.get(i-3).getData()) && str.contains(lexemes.get(i-2).getData()) && str.contains(lexemes.get(i-1).getData())){
					...
					else if(i-2>=0 && str.contains(lexemes.get(i-2).getData()) && str.contains(lexemes.get(i-1).getData())){
						data += lexemes.get(i-2).getData() + " ";
						data += lexemes.get(i-1).getData() + " ";
						lexemes.remove(i-1);
						lexemes.remove(i-2);
		...
		/*2. Merge Expression
			...
			2-2. find BracketExpression from expressions and make them compoundExpression 
					ex. (, a, b, c, ) = bracketExpression, idExpression, idExpression, idExpression, bracketExpression -> CompoundExpression 
																															(in CompoundExpression, expressions = {bracketExpression, idExpression, idExpression, idExpression, bracketExpression}) 
			2-3. ArrayExpression Index merging
					ex. arrayExpression, compoundExpression(include []), compoundExpression(include []) -> arrayExpression (in arrayExpression, indexexpression = {compoundExpression, compoundExpression})
			2-4. malloc library call Exception (has 2 parameter before and after)
			2-5. CallExpression merging
					ex. CallExpression, compoundExpression(include ()) -> CallExpression (in callExpression, actualParameters = {compoundExpression})
			2-6. Unary, Binary merging
					ex. !, k = unaryExpression, idExpression -> UnaryExpression (operator: !, operand: k, position: true)
					ex. a, =, b = idExpression, BinaryExpression, idExpression -> BinaryExpression (operator: =, lhsOperand: a, rhsOperand: b)
		...
		*/	
		...
		for (int i = 0; i < expressions.size();) {
			if (expressions.get(i) instanceof CastExpression){
				while(i-1>=0 && expressions.get(i-1) instanceof CastExpression){
					((CastExpression)expressions.get(i)).setType(((CastExpression)expressions.get(i-1)).getType().toString() + " " + ((CastExpression)expressions.get(i)).getType().toString());
					expressions.remove(i-1);
					i--;
				}
				if (i-1>=0 && i+1<expressions.size() && expressions.get(i-1) instanceof BracketExpression && expressions.get(i+1) instanceof BracketExpression){
					UnaryExpression unaryExpr = new UnaryExpression();
				...
				if (i-1>=0 && expressions.get(i-1) instanceof CastExpression && (((CastExpression)expressions.get(i-1)).getType().toString().contains("const") || ((CastExpression)expressions.get(i-1)).getType().toString().contains("static"))){
					((IdExpression) expressions.get(i)).setType(((CastExpression)expressions.get(i-1)).getType().toString());
				}
				i++;
			} else if (expressions.get(i) instanceof LiteralExpression) {
				i++;
			} else if (expressions.get(i) instanceof BracketExpression) {
				if (((BracketExpression)expressions.get(i)).getType().equals("{}")  && ((BracketExpression)expressions.get(i)).getPosition() == true){
				...
				else if (((BracketExpression)expressions.get(i)).getType().equals("[]") && ((BracketExpression)expressions.get(i)).getPosition() == true){
				...
				if (expressions.get(i) instanceof ControlExpression) { 
					if (((ControlExpression)expressions.get(i)).getFunctionalName().equals("continue") || ((ControlExpression)expressions.get(i)).getFunctionalName().equals("break") ){
		...
		for (int i = 0 ; i<expressions.size() ; i++){
			if (expressions.get(i) instanceof LibraryCallExpression){
				if(((LibraryCallExpression)expressions.get(i)).getFunctionalName().equals("malloc")){
					...
					((CompoundExpression)expressions.get(i+1)).setExpression(mergeCall(((CompoundExpression)expressions.get(i+1)).getExpression()));
		...
		/*2-7. return library call Exception (none parameters or has parameters)*/
		for (int i = 0 ; i<expressions.size() ; i++){
			if (expressions.get(i) instanceof CallExpression && ((CallExpression)expressions.get(i)).getFunctionalName().equals("return")){
	...
	public ArrayList<Expression> mergeIndex(ArrayList<Expression> expressions){
		...
		for (int i = 0 ; i<expressions.size() ; i++){
			if (expressions.get(i) instanceof ArrayExpression) {
				while(i+1 < expressions.size() && expressions.get(i+1) instanceof CompoundExpression && ((CompoundExpression)expressions.get(i+1)).getExpression().isEmpty() == false){
						if (((CompoundExpression)expressions.get(i+1)).getExpression().get(0) instanceof BracketExpression 
						&& ((BracketExpression)((CompoundExpression)expressions.get(i+1)).getExpression().get(0)).getType().equals("[]")){
	...
	*/			
 			...
 			if (expressions.get(i) instanceof CallExpression) { 
				if (((CallExpression)expressions.get(i)).getActualParameters().isEmpty()){
					if (((CallExpression)expressions.get(i)).getFunctionalName().equals("return")){
						continue;
					} else if (i+1<expressions.size() && expressions.get(i+1) instanceof CompoundExpression && ((CompoundExpression)expressions.get(i+1)).getExpression().size() != 0 && ((CompoundExpression)expressions.get(i+1)).getExpression().get(0) instanceof BracketExpression){
						//((CompoundExpression)expressions.get(i+1)).setExpression(mergeOperator(((CompoundExpression)expressions.get(i+1)).getExpression()));
						((CompoundExpression)expressions.get(i+1)).setExpression(mergeCall(((CompoundExpression)expressions.get(i+1)).getExpression()));
						((CallExpression)expressions.get(i)).addActualParameters(expressions.get(i+1));
						expressions.remove(i+1);
				...
				} else if (((CallExpression)expressions.get(i)).getActualParameters().get(0) instanceof CompoundExpression){
					mergeCall(((CompoundExpression)((CallExpression)expressions.get(i)).getActualParameters().get(0)).getExpression());
				} 
				...
				if (((BinaryExpression)expressions.get(i)).getLhsOperand() instanceof CompoundExpression){
					mergeCall(((CompoundExpression)((BinaryExpression)expressions.get(i)).getLhsOperand()).getExpression());
				}
				if (((BinaryExpression)expressions.get(i)).getRhsOperand() instanceof CompoundExpression){
					mergeCall(((CompoundExpression)((BinaryExpression)expressions.get(i)).getRhsOperand()).getExpression());
				}
			} else if (expressions.get(i) instanceof CompoundExpression){
				if (expressions.get(i) instanceof CallExpression && ((CallExpression)expressions.get(i)).getFunctionalName().equals("return")){
					continue;
				...
				if (i-1>=0 && expressions.get(i-1) instanceof CompoundExpression && ((CompoundExpression)expressions.get(i-1)).getExpression().size() != 0 && ((CompoundExpression)expressions.get(i-1)).getExpression().get(0) instanceof BracketExpression){//typedef exception 4 - only has nick name
					...
					else ((TypedefExpression)expressions.get(i)).setExpressions(((CompoundExpression)expressions.get(i-1)).getExpression());
					expressions.remove(i-1);
				}
				if (i+1<expressions.size() && expressions.get(i+1) instanceof CompoundExpression && ((CompoundExpression)expressions.get(i+1)).getExpression().size() != 0 && ((CompoundExpression)expressions.get(i+1)).getExpression().get(0) instanceof BracketExpression){
					if (((TypedefExpression)expressions.get(i)).getType().contains("enum")) {
						ArrayList<Expression> tmp = new ArrayList<>();
						tmp.add(((CompoundExpression)expressions.get(i+1)));
						((TypedefExpression)expressions.get(i)).setExpressions(tmp);
				...
				if (i+1<expressions.size() && expressions.get(i+1) instanceof CompoundExpression == false && ((TypedefExpression)expressions.get(i)).getType().contains("enum")){
					i = i+1;
	...
	public ArrayList<Expression> mergeOperator(ArrayList<Expression> expressions){
		for (int i = 0 ; i < expressions.size() ; i++){
			if (expressions.get(i) instanceof CastExpression) {
				if (i+4<expressions.size() && expressions.get(i+4) instanceof LiteralExpression && ((LiteralExpression)expressions.get(i+4)).getConstant().equals(";") ){
					continue;
				...
				if (((CallExpression)expressions.get(i)).getFunctionalName().equals("sizeof") || ((CallExpression)expressions.get(i)).getFunctionalName().equals("malloc")) continue;
				else mergeOperator(((CallExpression)expressions.get(i)).getActualParameters());
			}
			/*unary Expression handling */
			else if (expressions.get(i) instanceof UnaryExpression){
				if (i-1 >=0 && expressions.get(i-1) instanceof CastExpression && ((UnaryExpression)expressions.get(i)).getOperator().toString().equals("*") && i+1<expressions.size() && expressions.get(i+1) instanceof IdExpression){
				...
				else if (i-2>=0 && expressions.get(i-2) instanceof BracketExpression && i-1 >=0 && expressions.get(i-1) instanceof CastExpression && ((UnaryExpression)expressions.get(i)).getOperator().toString().equals("*") && i+1<expressions.size() && expressions.get(i+1) instanceof BracketExpression){
				...
				else if (i-1<0 || (expressions.get(i-1) instanceof IdExpression == false && expressions.get(i-1) instanceof ArrayExpression == false)) {
					Expression operand = expressions.remove(i + 1);
					if (operand instanceof CompoundExpression && operand instanceof BinaryExpression == false && operand instanceof UnaryExpression ==false ){
		...
		for (int i = 0 ; i<expressions.size() ; i++){
			if (expressions.get(i) instanceof BinaryExpression){
				if (((BinaryExpression)expressions.get(i)).getOperator().getOperator().equals("*") || ((BinaryExpression)expressions.get(i)).getOperator().getOperator().equals("/") || ((BinaryExpression)expressions.get(i)).getOperator().getOperator().equals("%")){
	 ...
	 * 2. if find end bracket -> brakcet from brackets stack pop + end bracket , add to allBrackets(LinkedHashMap<beginning, end>)
	 *  +) all: () and []
	 *     curly: {}
	 */
	public LinkedHashMap<Integer, Integer> findBrackets(ArrayList<Expression> expressions, String type){
		...
		if (type.equals("all")){ //()
			for (int i = 0 ; i<expressions.size() ; i++){
				if (expressions.get(i) instanceof BracketExpression && ((BracketExpression)expressions.get(i)).getType().equals("()") && ((BracketExpression)expressions.get(i)).getPosition() == true){
					brackets.add(i);
				} else if (expressions.get(i) instanceof BracketExpression && ((BracketExpression)expressions.get(i)).getType().equals("()") && ((BracketExpression)expressions.get(i)).getPosition() == false){
					allBrackets.put(brackets.pop(), i);
				} else if (expressions.get(i) instanceof BracketExpression && ((BracketExpression)expressions.get(i)).getType().equals("[]") && ((BracketExpression)expressions.get(i)).getPosition() == true){
					brackets.add(i);
				} else if (expressions.get(i) instanceof BracketExpression && ((BracketExpression)expressions.get(i)).getType().equals("[]") && ((BracketExpression)expressions.get(i)).getPosition() == false){
					allBrackets.put(brackets.pop(), i);
				}

...
def generate_assume_assert(s1_in_p, pred, s2_out_p, assumptions):
	...
	f.write("\n\
	// STATE / OUTPUT assume \n\
	__CPROVER_assume(prev_state.match >= 0 && prev_state.match <= 6); \n\
	__CPROVER_assume(prev_output.result >= 0 && prev_output.result <= 2); \n\
	")

	## Assume INPUT Define Section
	f.write("\n\
	// INPUT assume \n\
	__CPROVER_assume(prev_input.trianglea >= 1 && prev_input.trianglea <= 9); \n\
	__CPROVER_assume(input.trianglea >= 1 && input.trianglea <= 9); \n\
	__CPROVER_assume(prev_input.triangleb >= 1 && prev_input.triangleb <= 9); \n\
	__CPROVER_assume(input.triangleb >= 1 && input.triangleb <= 9); \n\
	__CPROVER_assume(prev_input.trianglec >= 1 && prev_input.trianglec <= 9); \n\
	__CPROVER_assume(input.trianglec >= 1 && input.trianglec <= 9); \n\
	")
	...
	{						\n\
		...
		// new input assume		\n\
		INPUT new_input;	\n\
		__CPROVER_assume(new_input.trianglea >= 1 && new_input.trianglea <= 9); \n\
		__CPROVER_assume(new_input.triangleb >= 1 && new_input.triangleb <= 9); \n\
		__CPROVER_assume(new_input.trianglec >= 1 && new_input.trianglec <= 9); \n\
		")

	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		state.match, " + ", ".join(targetNamesOutput) + ");\n")
	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		rt_state.match, " + ", ".join(targetNamesRTStructed) + ");\n")
	f.write("\t" + "printf(\"PROP: assert(!(state.match == %d && rt_state.match == %d && " + " && ".join(targetProp) + "));\", \n"\
		+ "prev_state.match, state.match, " + ", ".join(targetNamesPrevStructed) + ");\n")
	f.write("\t" + "printf(\"ASSUME: __CPROVER_assume(!(prev_state.match == %d && state.match == %d && " + " && ".join(targetAssume) + "));\", \n"\
		+ "prev_state.match, state.match, " + ", ".join(targetNamesPrevStructed) + ");\n")
...
targetLen = 4
targetNames = ["trianglea", "triangleb", "trianglec", "result"]
targetTypes = ["input", "input", "input", "output"]
targetExprs = ["%d", "%d", "%d", "%d"]
topElement = ["trianglea", "triangleb", "trianglec", "result"]

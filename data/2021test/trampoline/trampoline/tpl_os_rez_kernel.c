...
const tpl_resource_id RES_SCHEDULER = 3 - 1;
...
{ 
  /***********New Added Assert**********/
  //assert(res_id!=INVALID_RESOURCE);
  /***********New Added Assert**********/
  tpl_status result = 0;
  tpl_resource * res;
  tpl_get_task_lock();
  if(((tpl_bool)0)!=tpl_get_interrupt_lock_status()) { result = 19; }
  tpl_service.service_id = (12);
  tpl_service.parameters.id.res_id = (res_id);
  if ((result == (tpl_status)0) && ((res_id) >= ((tpl_resource_id)3))) { result = (tpl_status)3; }
  if ((result) == 0) {

...
def generate_assume_assert(s1_in_p, pred, s2_out_p, assumptions):
	...
	f.write("\n\
	// STATE / OUTPUT assume \n\
	__CPROVER_assume(prev_state.mode >= 0 && prev_state.mode <= 3); \n\
	__CPROVER_assume(prev_state.counter >= 0 && prev_state.counter <= 3); \n\
	")

	## Assume INPUT Define Section
	f.write("\n\
	// INPUT assume \n\
	__CPROVER_assume(prev_input.power_switch >= 0 && prev_input.power_switch <= 1); \n\
	__CPROVER_assume(input.power_switch >= 0 && input.power_switch <= 1); \n\
	")
	...
	{						\n\
		...
		// new input assume		\n\
		INPUT new_input;	\n\
		__CPROVER_assume(new_input.power_switch >= 0 && new_input.power_switch <= 1); \n\
		")

	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		state.mode, " + ", ".join(targetNamesOutput) + ");\n")
	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		rt_state.mode, " + ", ".join(targetNamesRTStructed) + ");\n")
	f.write("\t" + "printf(\"PROP: assert(!(state.mode == %d && rt_state.mode == %d && " + " && ".join(targetProp) + "));\", \n"\
		+ "prev_state.mode, state.mode, " + ", ".join(targetNamesPrevStructed) + ");\n")
	f.write("\t" + "printf(\"ASSUME: __CPROVER_assume(!(prev_state.mode == %d && state.mode == %d && " + " && ".join(targetAssume) + "));\", \n"\
		+ "prev_state.mode, state.mode, " + ", ".join(targetNamesPrevStructed) + ");\n")
...
def get_trace():
	...
	mode_str = ["OFF","READY_TO_ON","READY_TO_OFF","ON"]
...
targetLen = 2
targetNames = ["counter", "power_switch"]
targetTypes = ["state", "input"]
targetExprs = ["%d", "%d"]
topElement = ["counter", "power_switch"]

...
def plot_model(model,trace_input,num_states,hyperparams,nfa=True):
    ...
    # states = []
    ####################################################################################
    # 2023-07-18(Tue) SoheeJung
    # state collection modification
    # original code
    ####################################################################################
    # for i in model:
    ...
    state_check = []
    for i in model:
        state_check.append(i[0])
        state_check.append(i[2])
    state_check = list(np.unique(state_check))
   
    # state check value initialize 0
    ...
    # print(model)
    for i in model:
        state_name_label = event_uniq[i[1]-1]
        
        if ":" in state_name_label :
            state_name_label = state_name_label.split(":")[1].strip()
        
        # if state is already check, then remove that state's information and update that information.
        if state_check[i[2]-1] == 0 :
            outgoing_state = str(i[2]) + ":\"" + state_name_label + "\""
            state_check[i[2]-1] = 1 # check state information that already checking
        else :
            popIndex = 0
            for j in range(len(states)) :
                if (str(i[2])) in states[j] :
                    popIndex = j
            states.pop(popIndex)
            outgoing_state = str(i[2]) + ":\"" + state_name_label + "\""

        states.append(outgoing_state)
    states = list(np.unique(states))
    # print("states :", states)
    
    ####################### finish state collection modification #####################
    

    ####################################################################################
    # 2023-07-18(Tue) SoheeJung
    # transition collection modification
    # original code
    ####################################################################################
    # transitions = []
    ...
    for i in range(len(t)):
        if t[i][0] == 1:
            real_start_state = ""
            for s in states:
                # 2023-10-30(Mon) SoheeJung : real state has value of outgoing state, then split before colon
                tempstr = s.split(":")[0]
                if str(t[i][2]) in tempstr:
                    real_start_state = s
            temp_trans = [event_uniq[t[i][1]-1], '1', real_start_state]
            transitions.append(temp_trans)
        else :
            incoming_state = str(t[i][0])
            outgoing_state = str(t[i][2])
            for j in range(len(states)) :
                # 2023-10-30(Mon) SoheeJung : real state has value of outgoing state, then split before colon
                tempstr = states[j].split(":")[0]
                if str(t[i][0]) in tempstr :
                    incoming_state = states[j]
                if str(t[i][2]) in tempstr :
                    outgoing_state = states[j]
            # temp_trans = [event_uniq[t[i][1]-1],str(t[i][0]),str(t[i][2])]
            temp_trans = [event_uniq[t[i][1]-1], incoming_state, outgoing_state]
            transitions.append(temp_trans)
    print("transitions :", transitions)

    # for i in range(len(t)):
    #     if t[i][0] == 1:
    #         temp_trans = [event_uniq[t[i][1]-1], "start dummy", state_dict[t[i][2]]]
    #         transitions.append(temp_trans)
    #     elif t[i][0] in state_dict.keys():
    #         temp_trans = [event_uniq[t[i][1]-1], state_dict[t[i][0]], state_dict[t[i][2]]]
    #         transitions.append(temp_trans)
    #     else:
    #         temp_trans = [event_uniq[t[i][1]-1], str(t[i][0]), state_dict[t[i][2]]]
    #         transitions.append(temp_trans)
    # print("transitions :", transitions)

    name = trace_filename.replace('.txt','')
    name = name.split('/')[-1]
    predicate_file = open(target_model_path + "/" + name + "_predicate.txt", 'w')
    
    number_predicates = []
    for i in range(len(transitions)):
        predicate_file.write("(" + str(i + 1) + ")\n")
        predicate_file.write("outgoing state: " + transitions[i][1] + "\n")
        predicate_file.write("incoming state: " + transitions[i][2] + "\n")
        predicate_file.write("predicate: \n")
        predicate_file.write(transitions[i][0] + "\n\n")
        temp_number_predicate = [str(i + 1), transitions[i][1], transitions[i][2]]
        number_predicates.append(temp_number_predicate)

    model = vis_trace()
    machine = GraphMachine(model=model, 
                       states=states, 
                       transitions=number_predicates,
                       initial = '1',

#define TASK(n) void TASK_##n
#define FUNC(n, m) k n FUNC_##k

#include <assert.h>
// #include "elev.h"
#include "orders.h"
#include "model.h"


// int orderMatrix[N_BUTTONS][N_FLOORS] = {{0}}; // ordrematrise


/* 2023-01-26 외부에서 랜덤 값을 설정하기 위한 코드 제거. */
// void setOrdersHigh() {
// 	for(int button = 0; button <= BUTTON_COMMAND; button++) {
// 		for(int floorNum = 0; floorNum < N_FLOORS; floorNum++) {
// 			if (elev_get_button_signal(button, floorNum)) {
// 				orderMatrix[button][floorNum] = 1; // sett en bestillingscelle logisk høy
// 				elev_set_button_lamp(button, floorNum, 1); // lys opp knapper
// 			}
// 		}
// 	}
// }
/**/


void flushOrders(){ 
	for(int button = 0; button <= BUTTON_COMMAND; button++) {
		for(int floorNum = 0; floorNum < N_FLOORS; floorNum++) {
			rt_input.orderMatrix[button][floorNum] = 0; // sett bestillingscelle logisk false
			// elev_set_button_lamp(button, floorNum, 0); // skru av lys
		}
	}
}


int isButtonPressed(){
	for (int button = 0; button < N_BUTTONS; button++) {
		for (int floorNum = 0; floorNum < N_FLOORS; floorNum++) {
			if(rt_input.orderMatrix[button][floorNum] == 1 ) {
				return 1;
			}
		}
	}
	return 0;
}

// 엘레베이터의 direction을 정해주는 함수
// 
int setDirection(int currentFloor, int lastDirection){
	
	//assert(currentFloor >= 0);
    //assert(currentFloor < N_FLOORS);
	
	//viss vi er mellom etasjer etter emergency stop, 
	//regner vi ut en float som tilsvarer en mellomposisjon
	// 만약 currentFloorLocation이 emergency라면, currentFloor 값 설정
	if(rt_input.currentFloorLocation == -1){ 
		(float) currentFloor;
		currentFloor = currentFloor + lastDirection/2.0;
	}

	int upOrders = 0;
	int downOrders = 0;
	
	//sjekker om bestillingen er over eller under posisjonen
	for (int button = 0; button < N_BUTTONS; button++) {
		for(int floorNum = 0; floorNum < N_FLOORS; floorNum++) {
			if(rt_input.orderMatrix[button][floorNum] == 1) {
				// 만약 현재 층보다 높은 층의 버튼이 눌려있다면, 엘레베이터는 위로 올라가야 함.
				if(currentFloor < floorNum) {
					upOrders++;
				}
				// 만약 현채 층보다 낮은 층의 버튼이 눌려있다면, 엘레베이터는 아래로 내려가야 함.
				else{
					downOrders++;
				}
			}
		}
	}

	//sammenligner og gjør en prioritering
	// 만약 엘레베이터가 위로 올라가야 한다면 1을 반환
	// 만약 엘레베이터가 아래로 내려가야 한다면 -1을 반환
	if(upOrders > downOrders) {
		return 1;
	}
	else{ // default er å kjøre ned 
		return -1;
	}
}


int floorIsOrdered(int floorNum, int motorDir){
	
	int floorBeyond;
	// heispanelbestillinger utføres uansett
	if (rt_input.orderMatrix[2][floorNum] == 1) {
		return 1;
	}
	//viss heisen står i ro
	if(motorDir == 0){
		if((rt_input.orderMatrix[0][floorNum] == 1 ) || (rt_input.orderMatrix[1][floorNum] == 1))  {
			return 1;
		}
	}

	//viss heisen kjører opp
	if(motorDir == 1){
		floorBeyond = 0;
		for (int button = 0; button < 2; button++) {
			for(int i = floorNum + 1; i < N_FLOORS; i++){
				floorBeyond += rt_input.orderMatrix[button][i];
			}
		}
		if (rt_input.orderMatrix[0][floorNum] == 1) {
			return 1;
		}
		else if (rt_input.orderMatrix[1][floorNum] && (floorBeyond == 0)) {
			return 1;
		}
	}
	
	//viss heisen kjører ned
	if(motorDir == -1){
		floorBeyond = 0; // om det er bestillinger forbi etasjen den er i
		for (int button = 0; button < 2; button++) {
			for(int i = 0; i < floorNum; i++){
				floorBeyond += rt_input.orderMatrix[button][i];
			}
		}

		if (rt_input.orderMatrix[1][floorNum] == 1) {
			return 1;
		}
		else if (rt_input.orderMatrix[0][floorNum] && (floorBeyond == 0)) {
			return 1;
		}
	}	
	return 0;
}


void removeFromOrderMatrix(int floorNum){
	for (int button = 0; button < N_BUTTONS; button++) {
		rt_input.orderMatrix[button][floorNum] = 0;
		// elev_set_button_lamp(button, floorNum, 0);
	}
}
...
def generate_assume_assert(s1_in_p, pred, s2_out_p, assumptions):
	...
	f.write("\n\
	// STATE / OUTPUT assume \n\
	__CPROVER_assume(prev_state.machine_state >= 0 && prev_state.machine_state <= 4); \n\
	__CPROVER_assume(prev_state.timer >= 0 && prev_state.timer <= 5); \n\
	")

	## Assume INPUT Define Section
	f.write("\n\
	// INPUT assume \n\
	__CPROVER_assume(prev_input.amount >= 0 && prev_input.amount <= 2000); \n\
	__CPROVER_assume(input.amount >= 0 && input.amount <= 2000); \n\
	__CPROVER_assume(prev_input.item_num >= 0 && prev_input.item_num <= 3); \n\
	__CPROVER_assume(input.item_num >= 0 && input.item_num <= 3); \n\
	__CPROVER_assume(prev_input.buttonPress >= 0 && prev_input.buttonPress <= 1); \n\
	__CPROVER_assume(input.buttonPress >= 0 && input.buttonPress <= 1); \n\
	")
	...
	{						\n\
		...
		// new input assume		\n\
		INPUT new_input;	\n\
		__CPROVER_assume(new_input.amount >= 0 && new_input.amount <= 2000); \n\
		__CPROVER_assume(new_input.item_num >= 0 && new_input.item_num <= 3); \n\
		__CPROVER_assume(new_input.buttonPress >= 0 && new_input.buttonPress <= 1); \n\
		")

	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		state.machine_state, " + ", ".join(targetNamesOutput) + ");\n")
	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		rt_state.machine_state, " + ", ".join(targetNamesRTStructed) + ");\n")
	f.write("\t" + "printf(\"PROP: assert(!(state.machine_state == %d && rt_state.machine_state == %d && " + " && ".join(targetProp) + "));\", \n"\
		+ "prev_state.machine_state, state.machine_state, " + ", ".join(targetNamesPrevStructed) + ");\n")
	f.write("\t" + "printf(\"ASSUME: __CPROVER_assume(!(prev_state.machine_state == %d && state.machine_state == %d && " + " && ".join(targetAssume) + "));\", \n"\
		+ "prev_state.machine_state, state.machine_state, " + ", ".join(targetNamesPrevStructed) + ");\n")
	...
	if not next_states_pred:
		if not next_states_not_pred:
			...
			for i in range(len(next_states_not_pred)):
				...
				v_str = 'rt_state.machine_state == ' + s
				f.write("(" + v_str + ")")
		...
		for i in range(len(next_states_pred)):
			...
			v_str = 'rt_state.machine_state == ' + x
...
def get_trace():
	...
	item_str = ["COKE","PEPSI","WATER","JUICE"]
	machine_state_str = ["Initial","Insert","ButtonEnabled","Emit","Return"]
	...
	for line in lines:
		if "OUTPUT: " in line and "format" not in line:
			...
			temp[0] = machine_state_str[temp[0]]
...
targetLen = 4
targetNames = ["timer", "amount", "item_num", "buttonPress"]
targetTypes = ["state", "input", "input", "input"]
targetExprs = ["%d", "%d", "%d", "%d"]
topElement = ["timer", "amount", "item_num", "buttonPress"]

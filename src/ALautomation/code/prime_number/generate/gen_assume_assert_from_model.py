...
def generate_assume_assert(s1_in_p, pred, s2_out_p, assumptions):
	...
	f.write("\n\
	// STATE / OUTPUT assume \n\
	__CPROVER_assume(prev_state.result >= 0 && prev_state.result <= 1); \n\
	")

	## Assume INPUT Define Section
	f.write("\n\
	// INPUT assume \n\
	__CPROVER_assume(prev_input.n >= 1 && prev_input.n <= 10); \n\
	__CPROVER_assume(input.n >= 1 && input.n <= 10); \n\
	")
	...
	{						\n\
		...
		// new input assume		\n\
		INPUT new_input;	\n\
		__CPROVER_assume(new_input.n >= 1 && new_input.n <= 10); \n\
		")

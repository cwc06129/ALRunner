...
StatusType tpl_chain_task_service(
  const TaskType task_id)
{
  StatusType result = 0;
  tpl_get_task_lock();
  if(((tpl_bool)0)!=tpl_get_interrupt_lock_status()) { result = 19; }
  tpl_service.service_id = (2);
  tpl_service.parameters.id.task_id = (task_id);
  if ((result == (tpl_status)0) && (tpl_current_os_state() != (tpl_os_state)1)) { result = (tpl_status)2; }
  if ((result == (tpl_status)0) && (((task_id) >= (tpl_task_id)2) || ((task_id) < 0))) { result = (tpl_status)3; }
  if ((result == (tpl_status)0) && ((tpl_kern.running->resources) != ((void *)0))) { result = (tpl_status)6; }
  if ((result) == 0) {
    tpl_kern.running->activate_count--;
    result = tpl_activate_task(task_id);
    if (result == ((tpl_status)0 | (tpl_status)32))
    {
      tpl_schedule_from_dying();
      if (tpl_kern.need_switch != 0x0)
      {
        tpl_switch_context(((void *)0), &(tpl_kern.s_running->context));
      }
    }
    else
    {
...
StatusType tpl_get_task_id_service(
  TaskRefType task_id)
{
  StatusType result = 0;
  tpl_get_task_lock();
  if(((tpl_bool)0)!=tpl_get_interrupt_lock_status()) { result = 19; }
  tpl_service.service_id = (4);
  tpl_service.parameters.id.task_id_ref = (task_id);
  if ((result) == 0) {
 if (tpl_kern.running_id >= 0 && tpl_kern.running_id < 2)
 {
...
StatusType tpl_get_task_state_service(
  const TaskType task_id,

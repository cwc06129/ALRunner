...
def generate_assume_assert(s1_in_p, pred, s2_out_p, assumptions):
	...
	f.write("\n\
	// STATE / OUTPUT assume \n\
	__CPROVER_assume(prev_state.is_ModeManager >= 0 && prev_state.is_ModeManager <= 2); \n\
	__CPROVER_assume(prev_state.was_ModeManager >= 0 && prev_state.was_ModeManager <= 2); \n\
	__CPROVER_assume(prev_state.temporalCounter_i1 >= 0 && prev_state.temporalCounter_i1 <= 205); \n\
	")

	## Assume INPUT Define Section
	f.write("\n\
	// INPUT assume \n\
	__CPROVER_assume(prev_input.RadioReq >= 0 && prev_input.RadioReq <= 3); \n\
	__CPROVER_assume(input.RadioReq >= 0 && input.RadioReq <= 3); \n\
	__CPROVER_assume(prev_input.DiscEject >= 0 && prev_input.DiscEject <= 1); \n\
	__CPROVER_assume(input.DiscEject >= 0 && input.DiscEject <= 1); \n\
	")
	...
	{						\n\
		...
		// new input assume		\n\
		INPUT new_input;	\n\
		__CPROVER_assume(new_input.RadioReq >= 0 && new_input.RadioReq <= 3); \n\
		__CPROVER_assume(new_input.DiscEject >= 0 && new_input.DiscEject <= 1); \n\
		")

	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		state.is_ModeManager, " + ", ".join(targetNamesOutput) + ");\n")
	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		rt_state.is_ModeManager, " + ", ".join(targetNamesRTStructed) + ");\n")
	f.write("\t" + "printf(\"PROP: assert(!(state.is_ModeManager == %d && rt_state.is_ModeManager == %d && " + " && ".join(targetProp) + "));\", \n"\
		+ "prev_state.is_ModeManager, state.is_ModeManager, " + ", ".join(targetNamesPrevStructed) + ");\n")
	f.write("\t" + "printf(\"ASSUME: __CPROVER_assume(!(prev_state.is_ModeManager == %d && state.is_ModeManager == %d && " + " && ".join(targetAssume) + "));\", \n"\
		+ "prev_state.is_ModeManager, state.is_ModeManager, " + ", ".join(targetNamesPrevStructed) + ");\n")
	...
	if not next_states_pred:
		if not next_states_not_pred:
			...
			for i in range(len(next_states_not_pred)):
				...
				v_str = 'rt_state.is_ModeManager == ' + s
				f.write("(" + v_str + ")")
		...
		for i in range(len(next_states_pred)):
			...
			v_str = 'rt_state.is_ModeManager == ' + x
...
def run_code_proof():
	...
	p = subprocess.Popen('cbmc code/code_proof.goto --unwind 1 --object-bits 10 --trace > out_assume_assert.txt', stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
	try:
		output,o_err = p.communicate()
		p.kill()
		if o_err and 'error' in str(o_err):
...
def run_k_ind_base():
	p = subprocess.Popen('cbmc code/code_base --object-bits 10 > out_base.txt', stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
	try:
		output,o_err = p.communicate()
		p.kill()
		if o_err and 'error' in str(o_err):
...
targetLen = 4
targetNames = ["was_ModeManager", "temporalCounter_i1", "RadioReq", "DiscEject"]
targetTypes = ["state", "state", "input", "input"]
targetExprs = ["%d", "%d", "%d", "%d"]
topElement = ["was_ModeManager", "temporalCounter_i1", "RadioReq", "DiscEject"]
bottomElement = []

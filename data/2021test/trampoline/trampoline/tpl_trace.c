...
void tpl_trace_task_execute(
  TaskType new_executed_task_id)
{
  tpl_status new_executed_task_old_status;
  tpl_priority new_executed_task_prio;
...
void tpl_trace_task_preempt(
 tpl_kern_state kernel_stat)
{
  int preempted_task_id;
  tpl_status preempted_task_status;
  tpl_priority preempted_task_prio;
  int new_executed_task_id;
  tpl_status new_executed_task_old_status;
  tpl_priority new_executed_task_prio;
  if(tpl_kern.s_running->type != 0x2)
  {
...
void tpl_trace_task_terminate(
  const TaskType dying_task_id, const TaskType
chained_task_id)
{
  tpl_priority dying_task_prio;
  tpl_status chained_task_old_status;
  tpl_priority chained_task_prio;
  if(tpl_kern.s_running->type != 0x2)
  {
    tpl_trace_get_date();
...
{
  tpl_status task_old_status;
  tpl_priority task_prio;
  if(tpl_kern.s_running->type != 0x2)
  {
    tpl_trace_get_date();
...
void tpl_trace_task_wait(
  const TaskType waiting_task_id)
{
  tpl_status waiting_task_status;
  tpl_priority waiting_task_prio;
  if(tpl_kern.s_running->type != 0x2)
  {
    tpl_trace_get_date();
...
void tpl_trace_task_released(
  const TaskType released_task_id, const tpl_event_mask
event_id)
{
  tpl_status released_task_status;
  tpl_priority released_task_prio;
  if(tpl_kern.s_running->type != 0x2)
  {
    tpl_trace_get_date();
...
void tpl_trace_task_change_priority(
  const TaskType priority_changing_task_id)
{
  tpl_priority priority_changing_task_new_prio;
    if(tpl_kern.s_running->type != 0x2)
  {
    tpl_trace_get_date();
...
void tpl_trace_res_get(
  const tpl_resource_id res_id, const TaskType locking_entity_id)
{
  tpl_priority locking_entity_prio;
  tpl_trace_get_date();
...
void tpl_trace_isr_run(
  const tpl_isr_id running_isr_id)
{
  tpl_priority running_isr_prio;
  if(tpl_kern.s_running->type == 0x2)
  {
    tpl_trace_get_date();
...
void tpl_trace_isr_preempt(
  const tpl_kern_state kernel_stat)
{
  int preempted_isr_id;
  tpl_status preempted_isr_status;
  tpl_priority preempted_isr_prio;
  int new_executed_isr_id;
  tpl_status new_executed_isr_old_status;
  tpl_priority new_executed_isr_prio;
  if(tpl_kern.s_running->type == 0x2)
  {
    tpl_trace_get_date();
...
void tpl_trace_isr_terminate(
  const tpl_isr_id dying_isr_id, const tpl_isr_id
  chained_isr_id)
{
  tpl_priority dying_isr_prio;
  tpl_status chained_isr_old_status;
  tpl_priority chained_isr_prio;
  if(tpl_kern.s_running->type == 0x2)
  {
...
void tpl_trace_isr_activate(
  const tpl_isr_id isr_id)
{
  tpl_status isr_old_status;
  tpl_priority isr_prio;
...
void tpl_trace_isr_change_priority(
  const tpl_isr_id priority_changing_isr_id)
{
  tpl_priority priority_changing_isr_new_prio;
  if(tpl_kern.s_running->type == 0x2)
  {
    tpl_trace_get_date();
...
void tpl_trace_alarm_cancel(
  const tpl_alarm_id cancelled_alarm_id)
{
  tpl_trace_get_date();
  EVENT_BEGIN(16);

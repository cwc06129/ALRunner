...
public class ModelConstructor {
     ...
     * 	2-1. 0 index element : interest structure type (0 : INPUT / 1 : STATE / 2 : OUTPUT)
        ...
        while((contents = reader.readLine()) != null) {
        	...
        	if(contents.contains(">> struct name")) {
        		...
        		for(int i=0; i<str1.length - 1; i++) {
        			if(i == str1.length - 2) typeStr += str1[i];
        			else typeStr += str1[i] + " ";
        		}
...
//            }
     ...
     * @throws IOException
     * 1. print INPUT / STATE / OUTPUT structure (Even if there is no variable inside, the structure is created.)
     *  1-1. get variable hashset from main function, then compare variable's name to interest variable's name
     *  1-2. if there is same one, then print type and variable name.
...
//        		TypedefExpression typedefExpr = ((TypedefExpression)declStmt.getDeclExpression().get(0));
...
//        for(DeclarationStatement declStmt : udbInfoRetriever.getGlobals()) {
//        	// if declarationStatement in global part has 'enum' string, then make enum statement to model.h with some preprocessing.
//        	if(declStmt.getRawStringData().contains("enum")) {
//        		String declString = declStmt.getRawStringData().replace("typedefenum", "typedef enum ");
...
//        }
        		...
        		if(!var.getFileName().equals("NULL") && var.getScope().equals("Global")) {
        			if(var.getName().equals(enumName)) {
        				...
        				for(int i=0; i<getEnumMap().get(enumName).length; i++) {
        					if(i == getEnumMap().get(enumName).length - 1) writer.print(getEnumMap().get(enumName)[i]);
        					else writer.print(getEnumMap().get(enumName)[i] + ", ");
        				}
     ...
     // 2023-11-01(Wed) SoheeJung
        ...
        for(DeclarationStatement declstmt : udbInfoRetriever.getGlobals()) {
        	...
        	if(declExpr.getBinaryExpression().getLhsOperand() instanceof IdExpression) {
                ...
                if(getInputVarList().getNameList().contains(varIdExpr.getName())) {
                	if(!getArrMap().containsKey(varIdExpr.getName())) inputStructElement.add("\t" + varIdExpr.getType() + " " + varIdExpr.getName() + ";");
                	else {
                		String varType = varIdExpr.getType().toString().split(" ")[0];
                		inputStructElement.add("\t" + varType + " " + varIdExpr.getName() + "[" + String.valueOf(getArrMap().get(varIdExpr.getName())) + "];");
                	}
                }
                else {
        		...
        		if(getInputVarList().getNameList().contains(idExpr.getName())) {
                	if(!getArrMap().containsKey(idExpr.getName())) inputStructElement.add("\t" + idExpr.getType() + " " + idExpr.getName() + ";");
                	else {
                		String varType = idExpr.getType().toString().split(" ")[0];
                		inputStructElement.add("\t" + varType + " " + idExpr.getName() + "[" + String.valueOf(getArrMap().get(idExpr.getName())) + "];");
                	}
                }
        	}   		
        }
        ...
        for(DeclarationStatement declstmt : udbInfoRetriever.getGlobals()) {
        	...
        	if(declExpr.getBinaryExpression().getLhsOperand() instanceof IdExpression) {
	            ...
	            if(getStateVarList().getNameList().contains(varIdExpr.getName())) {
	            	if(!getArrMap().containsKey(varIdExpr.getName())) stateStructElement.add("\t" + varIdExpr.getType() + " " + varIdExpr.getName() + ";");
	            	else {
	            		String varType = varIdExpr.getType().toString().split(" ")[0];
	            		stateStructElement.add("\t" + varType + " " + varIdExpr.getName() + "[" + String.valueOf(getArrMap().get(varIdExpr.getName())) + "];");
	            	}
	            }
	            else {
                	// 2023-11-01(Tue) SoheeJung : struct variable processing code
        		...
        		if(getStateVarList().getNameList().contains(idExpr.getName())) {
                	if(!getArrMap().containsKey(idExpr.getName())) stateStructElement.add("\t" + idExpr.getType() + " " + idExpr.getName() + ";");
                	else {
                		String varType = idExpr.getType().toString().split(" ")[0];
                		stateStructElement.add("\t" + varType + " " + idExpr.getName() + "[" + String.valueOf(getArrMap().get(idExpr.getName())) + "];");
                	}
                }
        	}
        }
        ...
        for(DeclarationStatement declstmt : udbInfoRetriever.getGlobals()) {
        	...
        	if(declExpr.getBinaryExpression().getLhsOperand() instanceof IdExpression) {
	            ...
	            if(getOutputVarList().getNameList().contains(varIdExpr.getName())) {
	            	if(!getArrMap().containsKey(varIdExpr.getName())) outputStructElement.add("\t" + varIdExpr.getType() + " " + varIdExpr.getName() + ";");
	            	else {
	            		String varType = varIdExpr.getType().toString().split(" ")[0];
	            		outputStructElement.add("\t" + varType + " " + varIdExpr.getName() + "[" + String.valueOf(getArrMap().get(varIdExpr.getName())) + "];");
	            	}
	            }
	            else {
                	// 2023-11-01(Tue) SoheeJung : struct variable processing code
        		...
        		if(getOutputVarList().getNameList().contains(idExpr.getName())) {
                	if(!getArrMap().containsKey(idExpr.getName())) outputStructElement.add("\t" + idExpr.getType() + " " + idExpr.getName() + ";");
                	else {
                		String varType = idExpr.getType().toString().split(" ")[0];
                		outputStructElement.add("\t" + varType + " " + idExpr.getName() + "[" + String.valueOf(getArrMap().get(idExpr.getName())) + "];");
                	}
                }
        	}
        }
        ...
        // 2023-11-01(Wed) SoheeJung : struct variable addition to NameList of interest variable list
     ...
     * 1. collect DeclarationStatement from global statement and function CFG.
     * 2. make model_initialize function 
     *  2-1. DeclarationStatement -> ExpressionStatement
     *  2-2. [To-do] if there is no initial value, then using frama-c.
     *  2-3. add initializing expression statement to model_initailize function
     */
    public void makeModelInitialize(UdbInfoRetriever udbInfoRetriever) {
        ...
        // if variable's scope is local, then we should not initailize in model_initialize function.
        ...
        for(DeclarationStatement declstmt : declStmtList) {
            ...
            if(declstmt.getDeclExpression().get(0) instanceof DeclExpression) {
            	...
            	// if BinaryExpresion's rhs-operand is compoundExpression, then get LiteralExpressions and make each array declStatement
            	if(inputDeclstmt.getBinaryExpression().getRhsOperand() instanceof CompoundExpression) {
            		Integer arrIndex = 0; // check array index
            		for(Expression expr : ((CompoundExpression)(inputDeclstmt.getBinaryExpression().getRhsOperand())).getExpression()) {
            	...
            	// if BinaryExpression's lhs-operand is IdExpression that contains array element, then make each array declStatment assigned 0
            	...
            	// if BinaryExpression has no rhs-operand, then assign 0
            		...
            		// if rhsOperand is NullExpression and lhsOperand is ArrayExpression, then process this part
            		if(inputDeclstmt.getBinaryExpression().getLhsOperand().getCode().contains("[") && inputDeclstmt.getBinaryExpression().getLhsOperand().getCode().contains("]")) {
            			...
            			for(int i=0; i<indexNum; i++) {
            				...
            				CompoundExpression lhsExpr = new CompoundExpression(); // (example) arr[1]
            				// variable name
            				CompoundExpression cpdExpr = (CompoundExpression)(inputDeclstmt.getBinaryExpression().getLhsOperand());
            				IdExpression idExpr = (IdExpression)(cpdExpr.getExpression().get(0));
            		...
            		// if rhsOperand is NullExpression and lhsOperand is not ArrayExpression, then process this part
            		else {
            			boolean makeExprCheck = true;
            			BinaryExpression binExpr = new BinaryExpression(); // deep-copy
            			// 2023-11-01(Wed) SoheeJung : if variable is struct,then don't make expression statement
        ...
        for(int i = 0; i < exprStmtList.size(); i++) {
            if(i != 0) exprStmtList.get(i).addBefore(exprStmtList.get(i - 1));
            if(i != (exprStmtList.size() - 1)) exprStmtList.get(i).setNext(exprStmtList.get(i + 1));
        }
     ...
     * 2. make model_initialize function to initialize all the variable
     *    (save all the DeclarationStatement information, then make model_initialize function.)
     * 3. In the case of interest variables, it is changed from the original variable name to the structure variable name.
     *    (travse parsetree, and fine idExpression related with interest variable, then new annotation addition for them.)
     * 4. In the case of printf statement, make first and then copy and paste this statement whenever user want.
     */
    public void makeModelCFile(UdbInfoRetriever udbInfoRetriever, HashMap<Function, Integer> numberOfParam, HashMap<Function, Boolean> typeOfFunc, ArrayList<String> defines, String directoryPath) throws IOException {     
     ...
     *  2-1. If the above conditions are met, change idExpression.
     */
    public void modelCVariableChanger(UdbInfoRetriever udbInfoRetriever, String directoryPath) throws IOException {
        ...
        for (DeclarationStatement s : udbInfoRetriever.getGlobals()){ // global variable print
			for (Expression e : s.getDeclExpression()){
                ...
                if((e.getCode().contains("rt_input.")) || (e.getCode().contains("rt_state.")) || (e.getCode().contains("rt_output."))) continue;
                // 2023-10-19(Thu) SoheeJung
                // if Expression's instance is not TypedefExpression and Expression contains bracketExpression, then print specific position
                if(!(e instanceof TypedefExpression)) {
                	// if Expression contains bracketExpression
                	if(e.getCode().contains("[") && e.getCode().contains("]") && e.getCode().contains("{") && e.getCode().contains("}")) {
...
//        System.out.println(printfString);
        ...
        for(Function f : udbInfoRetriever.getFunctions()) {			
			codePrintTraverser.traverse(f, writer, getInputVarList(), getStateVarList(), getOutputVarList(), printfString);
			writer.println();
		}
        writer.println();
    ...
    public String makePrintfStatement(UdbInfoRetriever udbInfoRetriever, PrintWriter writer) {
    	...
    	for(Function function : udbInfoRetriever.getFunctions()) {
    		if(function.getName().equals("model_step")) modelStepFunction = function;
    	}
    	
    	// make printf statement to model_step last part
//    	PseudoStatement startPseudoStmt = (PseudoStatement)((CompoundStatement)(modelStepFunction.getBody())).getFirst();
        ...
        String variableStr = "";
        // 2023-05-25(Thu) SoheeJung
        // if key variable is checking, then make printf format and argument using second element
...
//        writer.print("printf(\"");
        ...
        // if key interest variable is output type, then print output variable first.
            ...
            for(Variable v : udbInfoRetriever.getVariables()) {
                if(getStateVarList().getNameList().get(0).equals(v.getName()) && !v.getFileName().equals("NULL") && v.getScope().equals("Global")) {
            ...
            for(Variable v : udbInfoRetriever.getVariables()) {
                if(getOutputVarList().getNameList().get(0).equals(v.getName()) && !v.getFileName().equals("NULL") && v.getScope().equals("Global")) {
                    if(v.getType().equals("int") || v.getType().equals("char") || v.getType().contains("int")) {
                        typeStr += "%d ";
                    	...
                    	if(getEnumMap().containsKey(v.getName())) {
                    		typeStr += "%s ";
                            variableStr += v.getName() + "_str[" + "rt_output." + getOutputVarList().getNameList().get(0) + "], ";
                    	}
                    }
                }
            }
        ...
        for(String s : getStateVarList().getNameList()) {
        	// 2023-11-02(Thu) SoheeJung : if interest variable is structure type, then don't make element string
        	boolean structCheck = false;
        	for(String structName : getStructVarScope().keySet()) {
        		if(structName.contains(s)) structCheck = true;
        	}
        	if(structCheck) continue;
        	/* struct type variable checking finish */
        	
            // 2023-05-25(Thu) SoheeJung
            // key variable checking -> if there is key variable, then make printf start at second element (not first element)
            ...
            for(Variable v : udbInfoRetriever.getVariables()) {
                if(s.equals(v.getName()) && !v.getFileName().equals("NULL") && v.getScope().equals("Global")) {
                	// 2023-10-23(Mon) SoheeJung
                	// if interest variable is array variable, then make printf type string iteration time
                	...
                	// if interest variable is enumeration type, then make printf with matching string array
        ...
        // 2023-11-02(Thu) SoheeJung : if variable is struct variable, then add this variable to element string seperatly
        for(String structName : getStructVarScope().keySet())      
        	if(structName.equals(getStructKeyVar())) continue;
        	else {
        ...
        // 2023-11-03(Fri) SoheeJung : if variable is constant type, then add this element to string
...
//        }
        ...
        // 2023-11-03(Fri) SoheeJung : if variable is constant type, then add this variable to element string
        for(String constStr : getConstList()) {
        ...
        if(getStructKeyVar() != null) {
    		if(getEnumMap().containsKey(getStructKeyVar())) {
				...
				for(int i=getStructVarScope().get(getStructKeyVar()).get(1); i<=getStructVarScope().get(getStructKeyVar()).get(2); i++) {
        ...
        for(String s : getInputVarList().getNameList()) {
        	...
        	if(s.contains(".")) {
        		if(getStructVarScope().get(s).get(0) >= 0) {
        			writer.println("\tin." + s + " = rand() % " + String.valueOf(getStructVarScope().get(s).get(1) - getStructVarScope().get(s).get(0) + 1) + " + " + String.valueOf(getStructVarScope().get(s).get(0)) + ";");
        		}
        		else {
        			writer.println("\tin." + s + " = rand() % " + String.valueOf(getStructVarScope().get(s).get(1) - getStructVarScope().get(s).get(0) + 1) + " " + String.valueOf(getStructVarScope().get(s).get(0)) + ";");
        		}
        	...
        	/* struct varaible processing finish */
        		...
        		for(Variable v : variables) {
        			...
        			for(String structStr : getStructMap().keySet()) {
        				if(structStr.contains(v.getType())) structCheck = true;
        			}
        			if(structCheck) break;
                    	...
                    	if(getArrMap().containsKey(v.getName())) {
                    		for(int i = 0; i < getArrMap().get(v.getName()); i++) {
                    			if(v.getVarScope().get(0) >= 0) {
                                    writer.println("\tin." + v.getName() + "[" + String.valueOf(i) + "] = rand() % " + String.valueOf(v.getVarScope().get(1) - v.getVarScope().get(0) + 1) + " + " + String.valueOf(v.getVarScope().get(0)) + ";");
                                } else {
                                    writer.println("\tin." + v.getName() + "[" + String.valueOf(i) + "] = rand() % " + String.valueOf(v.getVarScope().get(1) - v.getVarScope().get(0) + 1) + " " + String.valueOf(v.getVarScope().get(0)) + ";");
                                }
                    		}
                            ...
                            if(v.getVarScope().get(0) >= 0) {
                                writer.println("\tin." + v.getName() + " = rand() % " + String.valueOf(v.getVarScope().get(1) - v.getVarScope().get(0) + 1) + " + " + String.valueOf(v.getVarScope().get(0)) + ";");
                            } else {
                                writer.println("\tin." + v.getName() + " = rand() % " + String.valueOf(v.getVarScope().get(1) - v.getVarScope().get(0) + 1) + " " + String.valueOf(v.getVarScope().get(0)) + ";");
                            }
                    	}
    ...
    public void makeTraceShellFile(String srcPath, String directoryPath) throws IOException {
        ...
        //         return ((name.contains(".c") || name.contains(".h")) && !name.contains(".udb")); 
        //     }
        // };
     ...
     */
    private void makeGenAssumeAssertFromModelPyFile(String srcPath, HashSet<Variable> variables, String directoryPath) throws IOException {
        	...
        	for(Variable v : variables) {
        		if(s.equals(v.getName()) && !v.getFileName().equals("NULL") && v.getScope().equals("Global")) {
	        		if(v.getName().equals(s)) {
	        			if(getArrMap().containsKey(v.getName())) {
	        				for(int i=0; i<getArrMap().get(v.getName()); i++) {
	        					writer.println("\t__CPROVER_assume(prev_state." + s + "[" + String.valueOf(i)+ "] >= " + String.valueOf(v.getVarScope().get(0)) + " && prev_state." + s + "[" + String.valueOf(i)+ "] <= " + String.valueOf(v.getVarScope().get(1)) + "); \\n\\");
	        				}
	        			}
	        			else writer.println("\t__CPROVER_assume(prev_state." + s + " >= " + String.valueOf(v.getVarScope().get(0)) + " && prev_state." + s + " <= " + String.valueOf(v.getVarScope().get(1)) + "); \\n\\");
	        		}
        		}
        	}
        }
        	...
        	for(Variable v : variables) {
        		if(s.equals(v.getName()) && !v.getFileName().equals("NULL") && v.getScope().equals("Global")) {
	        		if(v.getName().equals(s)) {
	        			if(getArrMap().containsKey(v.getName())) {
	        				for(int i=0; i<getArrMap().get(v.getName()); i++) {
	        					writer.println("\t__CPROVER_assume(prev_output." + s + "[" + String.valueOf(i)+ "] >= " + String.valueOf(v.getVarScope().get(0)) + " && prev_output." + s + "[" + String.valueOf(i)+ "] <= " + String.valueOf(v.getVarScope().get(1)) + "); \\n\\");
	        				}
	        			}
	        			else writer.println("\t__CPROVER_assume(prev_output." + s + " >= " + String.valueOf(v.getVarScope().get(0)) + " && prev_output." + s + " <= " + String.valueOf(v.getVarScope().get(1)) + "); \\n\\");
	        		}
        		}
        	}
        }
        ...
        for(String structName : getStructVarScope().keySet()) {
        	if(getStructVarScope().get(structName).get(0) == 1) { // state
        		writer.println("\t__CPROVER_assume(prev_state." + structName + " >= " + String.valueOf(getStructVarScope().get(structName).get(1)) + " && prev_state." + structName + " <= " + String.valueOf(getStructVarScope().get(structName).get(2)) + "); \\n\\");
        	}
        	else if(getStructVarScope().get(structName).get(0) == 2) { // output
        		writer.println("\t__CPROVER_assume(prev_output." + structName + " >= " + String.valueOf(getStructVarScope().get(structName).get(1)) + " && prev_output." + structName + " <= " + String.valueOf(getStructVarScope().get(structName).get(2)) + "); \\n\\");
        	}
        }
...
//        }
        ...
        for(String s : getInputVarList().getNameList()) {
            ...
            for(Variable v : variables) {
                if(s.equals(v.getName()) && !v.getFileName().equals("NULL") && v.getScope().equals("Global")) {
                	if(getArrMap().containsKey(v.getName())) {
        				for(int i=0; i<getArrMap().get(v.getName()); i++) {
		                    writer.println("\t__CPROVER_assume(prev_input." + s + "[" + String.valueOf(i) + "] >= " + String.valueOf(v.getVarScope().get(0)) + " && prev_input." + s + "[" + String.valueOf(i) + "] <= " + String.valueOf(v.getVarScope().get(1)) + "); \\n\\");
		                    writer.println("\t__CPROVER_assume(input." + s + "[" + String.valueOf(i) + "] >= " + String.valueOf(v.getVarScope().get(0)) + " && input." + s + "[" + String.valueOf(i) + "] <= " + String.valueOf(v.getVarScope().get(1)) + "); \\n\\");
        				}
                	}
                	else {
            ...
            for(String structName : getStructVarScope().keySet()) {
            	if(getStructVarScope().get(structName).get(0) == 0) { // input
            		writer.println("\t__CPROVER_assume(prev_input." + structName + " >= " + String.valueOf(getStructVarScope().get(structName).get(1)) + " && prev_input." + structName + " <= " + String.valueOf(getStructVarScope().get(structName).get(2)) + "); \\n\\");
            		writer.println("\t__CPROVER_assume(input." + structName + " >= " + String.valueOf(getStructVarScope().get(structName).get(1)) + " && input." + structName + " <= " + String.valueOf(getStructVarScope().get(structName).get(2)) + "); \\n\\");
            	}
            }
            /* struct variable assume finish */
        }
        ...
        else if(getStateVarList().isKeyCheck()) writer.println("\t\tv_str = 'state." + getStateVarList().getNameList().get(0) + " == ' + s1");
        ...
        // 2023-05-25(Thu) SoheeJung
        // find key interest variable from state / output list, then make statement
        else if(getStateVarList().isKeyCheck()) writer.println("\t\t\t\tv_str = 'state." + getStateVarList().getNameList().get(0) + " == ' + s");
        ...
        for(String s : getInputVarList().getNameList()) {
            ...
            for(Variable v : variables) {
                if(s.equals(v.getName()) && !v.getFileName().equals("NULL") && v.getScope().equals("Global")) {
                	if(getArrMap().containsKey(v.getName())) {
        				for(int i=0; i<getArrMap().get(v.getName()); i++) {
        					writer.println("\t\t__CPROVER_assume(new_input." + s + "[" + String.valueOf(i) + "] >= " + String.valueOf(v.getVarScope().get(0)) + " && new_input." + s + "[" + String.valueOf(i) + "] <= " + String.valueOf(v.getVarScope().get(1)) + "); \\n\\");
        				}
                	}
                	else writer.println("\t\t__CPROVER_assume(new_input." + s + " >= " + String.valueOf(v.getVarScope().get(0)) + " && new_input." + s + " <= " + String.valueOf(v.getVarScope().get(1)) + "); \\n\\");
                }
            }
        }
        ...
        for(String structName : getStructVarScope().keySet()) {
        	if(getStructVarScope().get(structName).get(0) == 0) { // input
        		writer.println("\t\t__CPROVER_assume(new_input." + structName + " >= " + String.valueOf(getStructVarScope().get(structName).get(1)) + " && new_input." + structName + " <= " + String.valueOf(getStructVarScope().get(structName).get(2)) + "); \\n\\");
        	}
        }
        /* struct variable assume finish */
        writer.println("\t\t\")\n");
        ...
        if(getStructKeyVar() != null) {
        	if(getStructVarScope().get(getStructKeyVar()).get(0) == 1) { // state
        		...
        		writer.println("\t\toutput." + getStructKeyVar() + ", \" + \", \".join(targetNamesOutput) + \");\\n\")");
        	}
        }
        else if(getStateVarList().isKeyCheck()) {
...
//            }
        ...
        if(getStructKeyVar() != null) {
        	if(getStructVarScope().get(getStructKeyVar()).get(0) == 1) { // state
        		writer.println("\t\trt_state." + getStructKeyVar() + ", \" + \", \".join(targetNamesOutput) + \");\\n\")");
        	}
        	else if(getStructVarScope().get(getStructKeyVar()).get(0) == 2) { // output
        		writer.println("\t\trt_output." + getStructKeyVar() + ", \" + \", \".join(targetNamesOutput) + \");\\n\")");
        	}
        }
        else if(getStateVarList().isKeyCheck()) writer.println("\t\trt_state." + getStateVarList().getNameList().get(0) + ", \" + \", \".join(targetNamesRTStructed) + \");\\n\")");
        ...
        if(getStructKeyVar() != null) {
        	if(getStructVarScope().get(getStructKeyVar()).get(0) == 1) { // state
        		writer.println("\tf.write(\"\\t\" + \"printf(\\\"PROP: assert(!(state." + getStructKeyVar() + " == %d && rt_state." + getStructKeyVar() + " == %d && \" + \" && \".join(targetProp) + \"));\\\", \\n\"\\");
        	}
        	else if(getStructVarScope().get(getStructKeyVar()).get(0) == 2) { // output
        		writer.println("\tf.write(\"\\t\" + \"printf(\\\"PROP: assert(!(output." + getStructKeyVar() + " == %d && rt_output." + getStructKeyVar() + " == %d && \" + \" && \".join(targetProp) + \"));\\\", \\n\"\\");
        	}
        }
        else if(getStateVarList().isKeyCheck()) {
//            if(getTopElement().contains(getStateVarList().getNameList().get(0))) {
...
//            }
        ...
        if(getStructKeyVar() != null) {
        	if(getStructVarScope().get(getStructKeyVar()).get(0) == 1) { // state
        		writer.println("\t\t+ \"prev_state." + getStructKeyVar() + ", state." + getStructKeyVar() + ", \" + \", \".join(targetNamesPrevStructed) + \");\\n\")");
        	}
        	else if(getStructVarScope().get(getStructKeyVar()).get(0) == 2) { // output
        		writer.println("\t\t+ \"prev_output." + getStructKeyVar() + ", output." + getStructKeyVar() + ", \" + \", \".join(targetNamesPrevStructed) + \");\\n\")");
        	}
        }
        else if(getStateVarList().isKeyCheck()) {
//            if(getTopElement().contains(getStateVarList().getNameList().get(0))) {
...
//            }
        ...
        if(getStructKeyVar() != null) {
        	if(getStructVarScope().get(getStructKeyVar()).get(0) == 1) { // state
        		writer.println("\tf.write(\"\\t\" + \"printf(\\\"ASSUME: __CPROVER_assume(!(prev_state." + getStructKeyVar() + " == %d && state." + getStructKeyVar() + " == %d && \" + \" && \".join(targetAssume) + \"));\\\", \\n\"\\");
        	}
        	else if(getStructVarScope().get(getStructKeyVar()).get(0) == 2) { // output
        		writer.println("\tf.write(\"\\t\" + \"printf(\\\"ASSUME: __CPROVER_assume(!(prev_output." + getStructKeyVar() + " == %d && output." + getStructKeyVar() + " == %d && \" + \" && \".join(targetAssume) + \"));\\\", \\n\"\\");
        	}
        }
        else if(getStateVarList().isKeyCheck()) {
//            if(getTopElement().contains(getStateVarList().getNameList().get(0))) {
...
//            }
        ...
        // 2023-05-25(Thu) SoheeJung
        // find key interest variable from state / output list, then make statement
        else if(getStateVarList().isKeyCheck()) writer.println("\t\t\t\tv_str = 'rt_state." + getStateVarList().getNameList().get(0) + " == ' + s");
        ...
        // 2023-05-25(Thu) SoheeJung
        // find key interest variable from state / output list, then make statement
        else if(getStateVarList().isKeyCheck()) writer.println("\t\t\tv_str = 'rt_state." + getStateVarList().getNameList().get(0) + " == ' + x");
        ...
        writer.println("\tp = subprocess.Popen('cbmc code/code_proof.goto --unwind " + getUnwind() + " --object-bits " + getObject_bits() + " --trace > out_assume_assert.txt', stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)");
...
//        writer.println("\t## state_str = [?]\n");
        ...
        // 2023-10-24(Tue) SoheeJung
        // make tempInterest variable list (save all the interest variable to know index)
        ArrayList<String> tempInterestVarList = new ArrayList<String>();
        // 2023-11-02(Thu) SoheeJung : struct key variable check
        if(getStructKeyVar() != null) tempInterestVarList.add(getStructKeyVar());
        else if(getStateVarList().isKeyCheck()) tempInterestVarList.add(getStateVarList().getNameList().get(0));
        else if(getOutputVarList().isKeyCheck()) tempInterestVarList.add(getOutputVarList().getNameList().get(0));
        ...
        if(getStateVarList().isKeyCheck()) {
        	for(int i=1; i<getStateVarList().getNameList().size(); i++) {
            	...
            	boolean structCheck = false;
            	for(String structName : getStructVarScope().keySet()) {
            		if(structName.contains(getStateVarList().getNameList().get(i))) structCheck = true;
            	}
            	if(structCheck) continue;
            	/* struct type variable checking finish */
        		...
        		else tempInterestVarList.add(getStateVarList().getNameList().get(i));
        	}
        }
        else {
        ...
        for(int i=0; i<getInputVarList().getNameList().size(); i++) {
        	...
        	boolean structCheck = false;
        	for(String structName : getStructVarScope().keySet()) {
        		if(structName.contains(getInputVarList().getNameList().get(i))) structCheck = true;
        	}
        	if(structCheck) continue;
        	/* struct type variable checking finish */
    		...
    		else tempInterestVarList.add(getInputVarList().getNameList().get(i));
    	}
        ...
        if(getOutputVarList().isKeyCheck()) {
        	for(int i=1; i<getOutputVarList().getNameList().size(); i++) {
            	...
            	boolean structCheck = false;
            	for(String structName : getStructVarScope().keySet()) {
            		if(structName.contains(getOutputVarList().getNameList().get(i))) structCheck = true;
            	}
            	if(structCheck) continue;
            	/* struct type variable checking finish */
        		...
        		else tempInterestVarList.add(getOutputVarList().getNameList().get(i));
        	}
        }
        else {
        ...
        for(String structName : getStructVarScope().keySet()) {
        	if(structName.equals(getStructKeyVar())) continue;
        	else tempInterestVarList.add(structName);
        }
        ...
        for(int i=0; i<tempInterestVarList.size(); i++) {
        	if(getEnumMap().containsKey(tempInterestVarList.get(i))) {
        		writer.println("\t\t\ttemp[" + String.valueOf(i) + "] = " + tempInterestVarList.get(i) + "_str[temp[" + String.valueOf(i) + "]]");
        	}
        }
...
//        writer.println("\t\t\t## temp[0] = state_str[temp[0]] ?");
        ...
        // 2023-05-25(Thu) SoheeJung
        // find key interest variable from state / output list, then make statement
        else if(getStateVarList().isKeyCheck()) writer.println("\t\t\tstate." + getStateVarList().getNameList().get(0) + ",\" + ','.join(targetNamesOutput) + \");			\\n\")");
        ...
        if(getStructKeyVar() != null) {
        	if(getStructVarScope().get(getStructKeyVar()).get(0) == 1) { // state
        		writer.println("\t\t\trt_state." + getStructKeyVar() + ",\" + ','.join(targetNamesRTStructed) + \");			\\n\")");
        	}
        	else if(getStructVarScope().get(getStructKeyVar()).get(0) == 2) { // output
        		writer.println("\t\t\trt_output." + getStructKeyVar() + ",\" + ','.join(targetNamesRTStructed) + \");			\\n\")");
        	}
        }
        // 2023-05-25(Thu) SoheeJung
        // find key interest variable from state / output list, then make statement
        ...
        writer.println("\tp = subprocess.Popen('cbmc code/code_base --object-bits " + getObject_bits() + " > out_base.txt', stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)");
        ...
        // 2023-11-02(Thu) SoheeJung : struct key variable check
        if(getStructKeyVar() != null) {
        	if(getEnumMap().containsKey(getStructKeyVar())) writer.println("\t\ttemp = ['" + getEnumMap().get(getStructKeyVar())[0] + "']");
        	else writer.println("\t\ttemp = ['" + getStructVarScope().get(getStructKeyVar()).get(1) + "']");
        }
        // 2023-10-24(Tue) SoheeJung
        // key interest variable initialize
        else if(getStateVarList().isKeyCheck()) {
        	if(getEnumMap().containsKey(getStateVarList().getNameList().get(0))) writer.println("\t\ttemp = ['" + getEnumMap().get(getStateVarList().getNameList().get(0))[0] + "']");
        	else {
        		for(Variable v : variables) {
        			if(getStateVarList().getNameList().get(0).equals(v.getName())) {
        				if(!v.getFileName().equals("NULL") && v.getScope().equals("Global")) writer.println("\t\ttemp = ['" + v.getVarScope().get(0) + "']");
        			}
        		}
        	}
        }
        	...
        	if(getEnumMap().containsKey(getOutputVarList().getNameList().get(0))) writer.println("\t\ttemp = ['" + getEnumMap().get(getOutputVarList().getNameList().get(0))[0] + "']");
        	else {
        		for(Variable v : variables) {
...
//        Integer targetLen = getInputVarList().getNameList().size() + getStateVarList().getNameList().size() + getOutputVarList().getNameList().size() - 1 + getStructVarScope().keySet().size();
...
//            }
        ...
        // 2023-11-02(Thu) SoheeJung : top element print & bottom element print modification
...
//            }
//        }
//        topElementString = topElementString.substring(0, topElementString.length() - 2);
...
//        [Modify Version] : contain constant processing
        writer.println("targetNamesOutput = [targetNames[i] if targetTypes[i] == \"const\" else (\"\" if targetNames[i] in topElement else \"rt_\") + targetTypes[i] + \".\" + targetNames[i] for i in range(targetLen)]");
        writer.println("targetNamesRTStructed = [targetNames[i] if targetTypes[i] == \"const\" else (\"new_\" if targetTypes[i] == \"input\" else \"rt_\") + targetTypes[i] + \".\" + targetNames[i] for i in range(targetLen)]");
        writer.println("targetNamesStructedforProp = [targetNames[i] if targetTypes[i] == \"const\" else (\"\" if targetNames[i] in topElement else \"rt_\") + targetTypes[i] + \".\" + targetNames[i] for i in range(targetLen)]");
        writer.println("targetNamesPrevStructed = [targetNames[i] if targetTypes[i] == \"const\" else (\"prev_\" if targetNames[i] in topElement else \"\") + targetTypes[i] + \".\" + targetNames[i] for i in range(targetLen)]");
    ...
    public static void main(String[] args) throws IOException {
     ...
     // 2023-10-24(Tue) SoheeJung
        ...
        for(int i=0; i<originPath.length-1; i++) resultDirPath += originPath[i] + "/";
        	...
        	if(s.getRawStringData().contains("{...}")) {
        		try {
        			fileInputStream = new FileInputStream(file);
        	         ...
        	         String line = "";
        	         while((line = br.readLine()) != null) {
        	        	 if(line.contains("#define") || line.contains("#include")) continue;
        	        	 if(line.contains(varIdExpr.getName()) && line.contains("{") && line.contains("}") && line.contains(";")) {
        		...
        		try {
        			fileInputStream = new FileInputStream(file);
        	         ...
        	         while((line = br.readLine()) != null) {
        	        	 ...
        	        	 if(templine.charAt(0) == '/') continue;
        	        	 if(line.contains("#define") || line.contains("#include")) continue;
        	        	 if(line.contains(") {")) break; // if function is exist, then break. because after function that is not global section..
        	        	 ...
        	        	 if(lineList.contains(varIdExpr.getName()) && line.contains("=") && line.contains(";") && !line.contains("[") && !line.contains("]")) {
//        	        		 System.out.println(line);
        	        		 String[] str1 = line.split(";");
        	        		 line = str1[0];
        	        		 str1 = line.split(" ");
        	        		 for(int i=0; i<str1.length; i++) str1[i] = str1[i].strip();
...
//        	        		 System.out.println(declExpr.getBinaryExpression());
        	        	 ...
        	        	 else if((lineList.contains(varIdExpr.getName()) || (lineList.contains("*" + varIdExpr.getName()))) && line.contains(";") && !line.contains("[") && !line.contains("]") && !line.contains("(") && !line.contains(")")) {
//        	        		 System.out.println(line);
        	        		 String[] str1 = line.split(";");
        	        		 line = str1[0];
        	        		 str1 = line.split(" ");
...
//        	        		 System.out.println(declExpr.getBinaryExpression());
        	        	 ...
        	        	 else if(lineList.contains(varIdExpr.getName()) && line.contains(";") && line.contains("[") && line.contains("]")) {
//        	        		 System.out.println(line);
	    	        		 String[] str1 = line.split(";");
	    	        		 line = str1[0];
	    	        		 str1 = line.split(" ");
        ...
        modelConstructor.parseTxtInfo("./src/ALautomation/code/" + filename + "/spec.txt", udbInfoRetriever.getVariables());

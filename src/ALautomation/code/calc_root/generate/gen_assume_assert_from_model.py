...
def generate_assume_assert(s1_in_p, pred, s2_out_p, assumptions):
	...
	f.write("\n\
	// STATE / OUTPUT assume \n\
	__CPROVER_assume(prev_state.result >= 0 && prev_state.result <= 2); \n\
	__CPROVER_assume(prev_state.determinant >= -324 && prev_state.determinant <= 81); \n\
	")

	## Assume INPUT Define Section
	f.write("\n\
	// INPUT assume \n\
	__CPROVER_assume(prev_input.calca >= 0 && prev_input.calca <= 9); \n\
	__CPROVER_assume(input.calca >= 0 && input.calca <= 9); \n\
	__CPROVER_assume(prev_input.calcb >= 0 && prev_input.calcb <= 9); \n\
	__CPROVER_assume(input.calcb >= 0 && input.calcb <= 9); \n\
	__CPROVER_assume(prev_input.calcc >= 0 && prev_input.calcc <= 9); \n\
	__CPROVER_assume(input.calcc >= 0 && input.calcc <= 9); \n\
	")
	...
	{						\n\
		...
		// new input assume		\n\
		INPUT new_input;	\n\
		__CPROVER_assume(new_input.calca >= 0 && new_input.calca <= 9); \n\
		__CPROVER_assume(new_input.calcb >= 0 && new_input.calcb <= 9); \n\
		__CPROVER_assume(new_input.calcc >= 0 && new_input.calcc <= 9); \n\
		")

	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		state.result, " + ", ".join(targetNamesOutput) + ");\n")
	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		rt_state.result, " + ", ".join(targetNamesRTStructed) + ");\n")
	f.write("\t" + "printf(\"PROP: assert(!(state.result == %d && rt_state.result == %d && " + " && ".join(targetProp) + "));\", \n"\
		+ "prev_state.result, state.result, " + ", ".join(targetNamesPrevStructed) + ");\n")
	f.write("\t" + "printf(\"ASSUME: __CPROVER_assume(!(prev_state.result == %d && state.result == %d && " + " && ".join(targetAssume) + "));\", \n"\
		+ "prev_state.result, state.result, " + ", ".join(targetNamesPrevStructed) + ");\n")
...
targetLen = 4
targetNames = ["determinant", "calca", "calcb", "calcc"]
targetTypes = ["state", "input", "input", "input"]
targetExprs = ["%d", "%d", "%d", "%d"]
topElement = ["determinant", "calca", "calcb", "calcc"]

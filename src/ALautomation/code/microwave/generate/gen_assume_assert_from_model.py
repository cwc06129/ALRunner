...
def generate_assume_assert(s1_in_p, pred, s2_out_p, assumptions):
	...
	f.write("\n\
	// STATE / OUTPUT assume \n\
	__CPROVER_assume(prev_state.micro_state >= 0 && prev_state.micro_state <= 6); \n\
	__CPROVER_assume(prev_state.micro_time >= 0 && prev_state.micro_time <= 9); \n\
	")

	## Assume INPUT Define Section
	f.write("\n\
	// INPUT assume \n\
	__CPROVER_assume(prev_input.micro_time >= 0 && prev_input.micro_time <= 9); \n\
	__CPROVER_assume(input.micro_time >= 0 && input.micro_time <= 9); \n\
	__CPROVER_assume(prev_input.event >= 0 && prev_input.event <= 7); \n\
	__CPROVER_assume(input.event >= 0 && input.event <= 7); \n\
	")
	...
	{						\n\
		...
		// new input assume		\n\
		INPUT new_input;	\n\
		__CPROVER_assume(new_input.micro_time >= 0 && new_input.micro_time <= 9); \n\
		__CPROVER_assume(new_input.event >= 0 && new_input.event <= 7); \n\
		")

	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		state.micro_state, " + ", ".join(targetNamesOutput) + ");\n")
	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		rt_state.micro_state, " + ", ".join(targetNamesRTStructed) + ");\n")
	f.write("\t" + "printf(\"PROP: assert(!(state.micro_state == %d && rt_state.micro_state == %d && " + " && ".join(targetProp) + "));\", \n"\
		+ "prev_state.micro_state, state.micro_state, " + ", ".join(targetNamesPrevStructed) + ");\n")
	f.write("\t" + "printf(\"ASSUME: __CPROVER_assume(!(prev_state.micro_state == %d && state.micro_state == %d && " + " && ".join(targetAssume) + "));\", \n"\
		+ "prev_state.micro_state, state.micro_state, " + ", ".join(targetNamesPrevStructed) + ");\n")
...
def get_trace():
	...
	micro_state_str = ["Waiting","FullPower","HalfPower","SetTime","Enabled","Disabled","Operation"]
	event_str = ["None","full","half","openDoor","cancel","timer","closeDoor","StartPress"]
	...
	for line in lines:
		if "OUTPUT: " in line and "format" not in line:
			...
			temp[0] = micro_state_str[temp[0]]
			temp[3] = event_str[temp[3]]
...
targetLen = 3
targetNames = ["micro_time", "micro_time", "event"]

...
def generate_assume_assert(s1_in_p, pred, s2_out_p, assumptions):
	...
	f.write("\n\
	// STATE / OUTPUT assume \n\
	__CPROVER_assume(prev_output.result >= -1 && prev_output.result <= 4); \n\
	")

	## Assume INPUT Define Section
	f.write("\n\
	// INPUT assume \n\
	__CPROVER_assume(prev_input.arr[0] >= 0 && prev_input.arr[0] <= 4); \n\
	__CPROVER_assume(input.arr[0] >= 0 && input.arr[0] <= 4); \n\
	__CPROVER_assume(prev_input.arr[1] >= 0 && prev_input.arr[1] <= 4); \n\
	__CPROVER_assume(input.arr[1] >= 0 && input.arr[1] <= 4); \n\
	__CPROVER_assume(prev_input.arr[2] >= 0 && prev_input.arr[2] <= 4); \n\
	__CPROVER_assume(input.arr[2] >= 0 && input.arr[2] <= 4); \n\
	__CPROVER_assume(prev_input.arr[3] >= 0 && prev_input.arr[3] <= 4); \n\
	__CPROVER_assume(input.arr[3] >= 0 && input.arr[3] <= 4); \n\
	__CPROVER_assume(prev_input.arr[4] >= 0 && prev_input.arr[4] <= 4); \n\
	__CPROVER_assume(input.arr[4] >= 0 && input.arr[4] <= 4); \n\
	__CPROVER_assume(prev_input.key >= 0 && prev_input.key <= 4); \n\
	__CPROVER_assume(input.key >= 0 && input.key <= 4); \n\
	")
	...
	{						\n\
		...
		// new input assume		\n\
		INPUT new_input;	\n\
		__CPROVER_assume(new_input.arr[0] >= 0 && new_input.arr[0] <= 4); \n\
		__CPROVER_assume(new_input.arr[1] >= 0 && new_input.arr[1] <= 4); \n\
		__CPROVER_assume(new_input.arr[2] >= 0 && new_input.arr[2] <= 4); \n\
		__CPROVER_assume(new_input.arr[3] >= 0 && new_input.arr[3] <= 4); \n\
		__CPROVER_assume(new_input.arr[4] >= 0 && new_input.arr[4] <= 4); \n\
		__CPROVER_assume(new_input.key >= 0 && new_input.key <= 4); \n\
		")

	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		output.result, " + ", ".join(targetNamesOutput) + ");\n")
	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		rt_output.result, " + ", ".join(targetNamesRTStructed) + ");\n")
	f.write("\t" + "printf(\"PROP: assert(!(output.result == %d && rt_output.result == %d && " + " && ".join(targetProp) + "));\", \n"\
		+ "prev_output.result, output.result, " + ", ".join(targetNamesPrevStructed) + ");\n")
	f.write("\t" + "printf(\"ASSUME: __CPROVER_assume(!(prev_output.result == %d && output.result == %d && " + " && ".join(targetAssume) + "));\", \n"\
		+ "prev_output.result, output.result, " + ", ".join(targetNamesPrevStructed) + ");\n")
...
targetLen = 7
targetNames = ["arr[0]", "arr[1]", "arr[2]", "arr[3]", "arr[4]", "key", "result"]
targetTypes = ["input", "input", "input", "input", "input", "input", "output"]
targetExprs = ["%d", "%d", "%d", "%d", "%d", "%d", "%d"]
topElement = ["arr[0]", "arr[1]", "arr[2]", "arr[3]", "arr[4]", "key", "result"]
bottomElement = []

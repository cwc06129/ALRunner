...
def simplify_expr(expr, types):
	var = []
	for t in types[0]:
		var.append(t.split(':')[0])

	# simplify when variables are compound variables like struct
	replace_var = []
	if var:
		for v in var:
			if '.' in v and v in expr:
				replace_var.append([v,v.split('.')[-1]])
...
def post_process(output,next_event,data_type,trace_dict,enum_val,key):
	...
	new_c = []
	for c in cond:
		if any([x in c for x in ['+','-','*']]) and '==' not in c:
			new_c.append(simplify_expr(c, trace_dict['event_types'][key]))
		else:
			new_c.append(c)
	cond = new_c
	print(cond)
	...
	for i in trace_set.keys():
		...
		if(const_grammar):
			...
			for x in range(last_ind):
				if(data_type[x][1] == 'N'):
					data = [round(float(y[x])) for y in temp if y[last_ind]!=i and y[last_ind]!='-']
		...
		for j in next_event:
			...
			output = post_process(output,[0,1],data_type,trace_dict,enum_val,i)

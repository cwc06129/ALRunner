...
def generate_assume_assert(s1_in_p, pred, s2_out_p, assumptions):
	...
	f.write("\n\
	// STATE / OUTPUT assume \n\
	__CPROVER_assume(prev_state.telestate >= 0 && prev_state.telestate <= 8); \n\
	__CPROVER_assume(prev_state.timer >= 0 && prev_state.timer <= 6); \n\
	")

	## Assume INPUT Define Section
	f.write("\n\
	// INPUT assume \n\
	__CPROVER_assume(prev_input.teleevent >= 0 && prev_input.teleevent <= 6); \n\
	__CPROVER_assume(input.teleevent >= 0 && input.teleevent <= 6); \n\
	__CPROVER_assume(prev_input.dialDigit >= 0 && prev_input.dialDigit <= 5); \n\
	__CPROVER_assume(input.dialDigit >= 0 && input.dialDigit <= 5); \n\
	")
	...
	{						\n\
		...
		// new input assume		\n\
		INPUT new_input;	\n\
		__CPROVER_assume(new_input.teleevent >= 0 && new_input.teleevent <= 6); \n\
		__CPROVER_assume(new_input.dialDigit >= 0 && new_input.dialDigit <= 5); \n\
		")

	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		state.telestate, " + ", ".join(targetNamesOutput) + ");\n")
	f.write("\t" + "printf(\"OUTPUT: %d " + " ".join(targetExprs) + "\\n\", \n\
		rt_state.telestate, " + ", ".join(targetNamesRTStructed) + ");\n")
	f.write("\t" + "printf(\"PROP: assert(!(state.telestate == %d && rt_state.telestate == %d && " + " && ".join(targetProp) + "));\", \n"\
		+ "prev_state.telestate, state.telestate, " + ", ".join(targetNamesPrevStructed) + ");\n")
	f.write("\t" + "printf(\"ASSUME: __CPROVER_assume(!(prev_state.telestate == %d && state.telestate == %d && " + " && ".join(targetAssume) + "));\", \n"\
		+ "prev_state.telestate, state.telestate, " + ", ".join(targetNamesPrevStructed) + ");\n")
...
def get_trace():
	...
	telestate_str = ["Idle","DialTone","TimeOut","Dialing","Connecting","Invalid","Busy","Ringing","Talking"]
	teleevent_str = ["None","CallerLift","busy","Connected","CalleeAnswer","CallerHangUp","DialButton"]
	...
	for line in lines:
		if "OUTPUT: " in line and "format" not in line:
			...
			temp[0] = telestate_str[temp[0]]
			temp[2] = teleevent_str[temp[2]]
...
targetLen = 3
targetNames = ["timer", "teleevent", "dialDigit"]
targetTypes = ["state", "input", "input"]
targetExprs = ["%d", "%d", "%d"]
topElement = ["timer", "teleevent", "dialDigit"]
